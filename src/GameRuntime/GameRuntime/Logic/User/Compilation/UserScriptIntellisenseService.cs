...
public sealed class UserScriptIntellisenseService
{
    public IReadOnlyList<UserScriptDiagnosticDto> GetDiagnostics(string userCode)
    {
        ...
        IEnumerable<UserScriptDiagnosticDto> roslynDiagnostics = compilation.GetDiagnostics()
            .Where(d => d.Severity == DiagnosticSeverity.Error || d.Severity == DiagnosticSeverity.Warning)
            .Select(MapDiagnostic)
            .Where(x => x is not null)
            .Cast<UserScriptDiagnosticDto>();

        IEnumerable<UserScriptDiagnosticDto> securityDiagnostics = UserScriptSecurityValidator.Validate(compilation, syntaxTree)
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (CompletionItem item in completionList.ItemsList.Take(50))
        {
