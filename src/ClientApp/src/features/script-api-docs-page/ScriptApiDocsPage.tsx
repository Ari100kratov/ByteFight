          ...
          <ScrollArea className="min-h-0 flex-1">
            <div className="space-y-2 p-2 pb-6">
              {searchResults.map(({ type, matches }) => (
                <button
                  key={type.fullName}
                  onClick={() => setSelectedType(type.fullName)}
                  className={[
              ...
              {searchResults.length === 0 && (
                <div className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
                  Ничего не найдено. Попробуй искать по названию типа, свойства,
                  метода, enum-значения или описанию.
                </div>
              )}
            </div>
          </ScrollArea>
          ...
          <div className="border-b bg-background/80 px-6 py-5 backdrop-blur">
            <div className="max-w-5xl">
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
                Автоматически собираемый справочник типов, свойств, методов и enum,
                доступных в пользовательском C#-коде.
                <br />
                Документация синхронизируется напрямую с игровым API и обновляется
