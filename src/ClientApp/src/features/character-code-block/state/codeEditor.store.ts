...
export const useCodeEditorStore = create<CodeEditorState>((set, get) => ({
  ...
  replaceFromServer: (codes) => {
    set((state) => {
      const keepActive = state.activeCodeId && codes.some((c) => c.id === state.activeCodeId)
      ...
      codes: state.codes.map((c) =>
        c.id === id
          ? {
              ...c,
              sourceCode,
              status: c.status === ChangeStatus.Created ? c.status : ChangeStatus.Updated,
            }
          : c,
      ),
    }))
      ...
      return {
        codes:
          code.status === ChangeStatus.Created
            ...
            : state.codes.map((c) => (c.id === id ? { ...c, status: ChangeStatus.Deleted } : c)),
        activeCodeId:
          state.activeCodeId === id ? state.codes.find((c) => c.id !== id)?.id : state.activeCodeId,
      }
    })
  },
